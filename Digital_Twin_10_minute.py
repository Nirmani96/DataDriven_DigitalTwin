...
def mae(forecast, actual):
    """
    Mean Absolute Error — average absolute difference per 10-min step.
    Lower is better. Same units as the data (patient counts).
    """
...
def rmse(forecast, actual):
    ...
    Useful for detecting replications with occasional large spikes.
    Lower is better.
    """
    f = np.array(forecast, dtype=float)
    a = np.array(actual,   dtype=float)
...
def mape_step(forecast, actual):
    """
    Step-level Mean Absolute Percentage Error.
    Only computed at steps where actual > 0 (avoids division by zero).
    Returns percentage (e.g. 25.0 = 25%).
    """
    f    = np.array(forecast, dtype=float)
    a    = np.array(actual,   dtype=float)
    mask = a > 0
...
if __name__ == "__main__":
    ...
    # ------------------------------------------------------------------
    # STEP 4: Compute per-replication validation metrics and save CSV
    # ------------------------------------------------------------------
    # This CSV is the key output used by Compare.py and
    # Compare_Discharge.py to statistically compare the Digital Twin
    # against the DES baseline.
    #
    # For each of the 100 replications, the following metrics are
    # computed by comparing that replication's 72-step forecast series
    # against the actual observed 72-step series:
    #
    ...
    print("\nComputing per-replication metrics…")
    per_run_metrics = []
    ...
    for run_id in sim["run"].unique():
        ...
        per_run_metrics.append({
            ...
            "Aggregate_PE_arrivals":      mape_aggregate(
                                              forecast_arr.sum(),
                                              actual_arr.sum()),
            "Aggregate_PE_discharges":    mape_aggregate(
                                              forecast_dis.sum(),
                                              actual_dis.sum()),
            "DTW_arrivals":               round(dtw_similarity(
                                              actual_arr, forecast_arr), 3),
            "DTW_discharges":             round(dtw_similarity(
                                              actual_dis, forecast_dis), 3),
        })
    ...
    print(f"Per-replication CSV saved: "
          f"{OUTPUT_DIR / 'dt_fixed_validation_per_replication_10min.csv'}")

    # Print mean metrics summary across all replications
    print("\n" + "=" * 55)
    print("MEAN METRICS ACROSS 100 REPLICATIONS (12h @ 10-min)")
    print("=" * 55)
    for col in [
        ...
        print(f"  {col:<36}: {val:.4f}{note}")

    # ------------------------------------------------------------------
    # STEP 5: Compute CI coverage and aggregate validation metrics
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # STEP 6: Compute DTW on TRUE MEDIAN forecast vs actual
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # STEP 7: Save aggregate validation metrics CSV
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # STEP 8: Save actual vs simulated time-series CSVs
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # STEP 9: Aggregate to hourly resolution for hourly plots
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # STEP 10: Generate and save all four plots
    # ------------------------------------------------------------------
    print("\nGenerating plots…")
